...
const MAPBOX_TOKEN = process.env.NEXT_PUBLIC_MAPBOX_TOKEN || "sk.eyJ1IjoibXV0aGFzc2ltNCIsImEiOiJjbWcyaW5zOTgxMTRyMmtzOTQydDNjbzN1In0.GzRk_OFR53CrS2r6cspn-w";
...
function getFallbackCoordinates(zipCode: string): { lon: number; lat: number } | null {
  const fallbackCoords: { [key: string]: { lon: number; lat: number } } = {
    ...
    "10003": { lon: -73.990, lat: 40.7328 }, // New York, NY
    ...
    "90211": { lon: -118.380, lat: 34.0505 }, // Beverly Hills, CA
    "90212": { lon: -118.380, lat: 34.0505 }, // Beverly Hills, CA
    ...
    "75201": { lon: -96.7970, lat: 32.7767 }, // Dallas, TX
    "75202": { lon: -96.7970, lat: 32.7767 }, // Dallas, TX
    "75203": { lon: -96.7970, lat: 32.7767 }, // Dallas, TX
...
async function zipToBoundingBox(zipCode: string): Promise<{ minx: number; miny: number; maxx: number; maxy: number } | null> {
  try {
...
async function zipToBoundingBoxAlternative(zipCode: string): Promise<{ minx: number; miny: number; maxx: number; maxy: number } | null> {
  try {
...
function generateRandomCoords(bbox: { minx: number; miny: number; maxx: number; maxy: number }): { lon: number; lat: number } {
...
async function coordsToAddresses(lon: number, lat: number, limit: number = 5): Promise<string[]> {
  try {
  ...
  try {
    ...
    if (!zip || typeof zip !== "string") {
      return NextResponse.json(
        { success: false, error: "ZIP কোড প্রয়োজন" },
        { status: 400 }
      );
    }
    ...
    if (!/^\d+$/.test(zip)) {
      return NextResponse.json(
        { success: false, error: "ZIP কোড শুধুমাত্র সংখ্যা হতে পারে" },
        { status: 400 }
      );
    }
    ...
    if (!bbox) {
      ...
      if (fallbackCoords) {
        ...
        return NextResponse.json(
          { success: false, error: "ZIP কোড রেজলভ করতে পারছি না" },
          { status: 400 }
        );
      }
    }
    ...
    if (uniqueAddresses.length === 0) {
      return NextResponse.json(
        { success: false, error: "কোনো এড্রেস পাওয়া যায়নি" },
        { status: 400 }
      );
    }
